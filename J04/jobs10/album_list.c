...
void push_front_album(AlbumItem **list, AlbumItem *new_item) {
    new_item->next = *list;
    *list = new_item;
}
...
int main() {
    ...
    push_back_album(&album1, album2);
    push_back_album(&album1, album3);
    ...
    push_front_album(&album1, create_album_item("Queen", "A Night at the Opera", 1975));
    printf("Albums after adding to the front:\n");
    current = album1;
    while (current != NULL) {
        printf("%s - %s (%d)\n", current->artist, current->title, current->year);
        current = current->next;
